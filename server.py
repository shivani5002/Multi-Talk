...
#         # Test if pipeline was created properly
#         if wan_pipeline is None:
#             raise Exception("WAN pipeline creation returned None")
            
#         logger.info("✅ WAN models initialized successfully!")
        
#     except Exception as e:
#         logger.error(f"❌ Error initializing models: {e}")
#         logger.error("This is a critical error - models failed to load!")
#         # Re-raise to make it clear the app cannot start
#         raise
...
#         # Check if wan_pipeline is properly initialized
#         if wan_pipeline is None:
#             error_msg = "❌ wan_pipeline is None! Models not loaded properly."
#             logger.error(error_msg)
#             raise Exception(error_msg)
        
#         # Additional check to ensure pipeline has required attributes
#         if not hasattr(wan_pipeline, 'use_teacache'):
#             error_msg = "❌ wan_pipeline missing required attributes - incomplete initialization"
#             logger.error(error_msg)
#             raise Exception(error_msg)
...
#         logger.info(f"   Audio type: {input_data.get('audio_type', 'N/A')}")
...
#         # Use streaming mode like your original working command
#         video = wan_pipeline.generate(
#             input_data,
#             size_buckget="multitalk-480",
#             motion_frame=25,
#             frame_num=81,
#             shift=2,  # --sample_shift 2
#             sampling_steps=8,  # --sample_steps 8
#             text_guide_scale=1.0,  # --sample_text_guide_scale 1.0
#             audio_guide_scale=2.0,  # --sample_audio_guide_scale 2.0
#             seed=42,
#             offload_model=True,
#             max_frames_num=1000,  # streaming mode uses 1000
#             mode="streaming",  # Add mode parameter like your original
...
#         # You might want to save the error to a file for this specific job
#         error_file = f"{output_path}_error.txt"
#         with open(error_file, 'w') as f:
#             f.write(f"Job {job_id} failed with error:\n{str(e)}\n\nTraceback:\n{traceback.format_exc()}")
...
def generate_video_worker(input_data, output_path, job_id):
    ...
    try:
        ...
        video = wan_pipeline.generate(
            ...
            max_frames_num=1000,
            color_correction_strength=1.0,
            extra_args=extra_args  # ✅ ADD THIS
        )
